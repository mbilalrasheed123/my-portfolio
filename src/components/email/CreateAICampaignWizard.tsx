...
export default function CreateAICampaignWizard({ onBack, onSuccess }: CreateAICampaignWizardProps) {
  ...
  const handleStepNext = () => {
    ...
    if (step === 1) {
      ...
      if (!csvContent) {
        setError("Please drag and drop or upload a valid Lead CSV file.");
        return;
      }
      if (!validationStats || validationStats.valid === 0) {
        setError("The uploaded CSV does not contain any rows with valid emails.");
        return;
      }
      setStep(4);
    }
        ...
        <div className="flex gap-3">
          ...
          {step < 4 ? (
            <button
              ...
              className="px-5 py-2 bg-[#2563eb] hover:bg-blue-600 active:scale-95 text-white rounded-xl font-mono text-[10px] uppercase tracking-widest transition-all flex items-center gap-2 cursor-pointer shadow-[0_4px_12px_rgba(37,99,235,0.25)] font-bold"
            >
